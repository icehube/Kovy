...
class FantasyAuction:
    ...
    def build_model(self):
        ...
        for i, row in self.filtered_df.iterrows():
            ...
            player_vars[i] = self.model.addVar(vtype="B", name=f"{player_name}_{player_position}")
        
        p = {'F' : 1 / 12, 'D' : 1 / 6, 'G' : 1 / 3}

        self.model.setObjective(
            sum(row['Pts'] * player_vars[i] * p[row['Pos']] for i, row in self.filtered_df.iterrows()),
            "maximize"
        )

        self.add_constraints(player_vars)
        self.solve_model()
        idxs = []
        for i, row in self.filtered_df.iterrows():
            player_name = row['Player']
            if self.model.getVal(player_vars[i]) == 1:
                idxs.append(i)
        print(self.filtered_df.loc[idxs])
    ...
    def write_to_csv(self):
        ...
        file_path = f'd:/Dropbox/equus/sillinger/app/players-{current_time}.csv'
